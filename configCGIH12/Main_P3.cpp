...
int main() {
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		ourShader.Use();  //manda a llamar a los shaders
		...
		//Cubo1
		view = glm::translate(view, glm::vec3(0.0f,0.0f,-10.0f)); //Aplicando una transformacion a la vista
		model = glm::rotate( model, 0.5f, glm::vec3( 1.0f, 0.0f, 0.0f ) ); // use to compare orthographic and perspective projection	rotar
		model = glm::scale(model, glm::vec3(1.0f, 1.0f, 1.0f));	//tansformación de escala
		//view = glm::translate( view, glm::vec3( screenWidth / 2, screenHeight / 2,-700.0f ) ); // use with orthographic projection, una transformacion se aplico a la vista
		...
		glBindVertexArray(VAO);
		glDrawArrays(GL_TRIANGLES, 0, 36);	//Dibujando triangulos desde el vertice 0 al 36
		glBindVertexArray(0);
